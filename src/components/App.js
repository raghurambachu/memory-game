  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (cardsFlipped.length === 2) {
      ...
      if (first.cid === second.cid) {
        ...
        this.setState({ cards: newCards, cardsFlipped: [], move });
      } else {
        setTimeout(() => {
          newCards = newCards.map((card) => {
            if (
          ...
          <ul className="flex justify-between flex-wrap gap-4 p-4 bg-gray-500 shadow-2xl ">
            {cards.map((card) => {
              return card.isFlipped ? (
                <li
                  key={card.id}
                  className="w-32 h-32 bg-blue-400 flex items-center justify-center shadow-md "
                >
                  {card.icon}
                </li>
              ) : (
                <li
                  key={card.id}
                  className="w-32 h-32 bg-gray-900 shadow-md cursor-pointer"
                  onClick={() => this.showCard({ id: card.id, cid: card.cid })}
                ></li>
              );
            })}
          </ul>
